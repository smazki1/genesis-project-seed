            ...
            <TabsList className="flex flex-wrap justify-center w-full mb-8 bg-white shadow-md rounded-lg overflow-hidden border border-gray-100">
              {useCases.map((useCase) => (
                <TabsTrigger 
                  key={useCase.id} 
                  value={useCase.id} 
                  className="flex items-center gap-2 py-4 px-6 data-[state=active]:bg-[#8B1E3F] data-[state=active]:text-white transition-all duration-200 hover:bg-gray-50 data-[state=active]:hover:bg-[#8B1E3F]/90"
                >
                      ...
                      <div className="flex flex-col md:flex-row">
                        ...
                        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center bg-white">
                          <div className="flex items-center gap-3 mb-4">
                            <div className="bg-[#8B1E3F]/10 p-2.5 rounded-full">
                              <useCase.icon className="w-5 h-5 text-[#8B1E3F]" />
                            </div>
                          ...
                          {useCase.hasCallout && (
                            <div className="mt-auto bg-[#F3752B]/10 p-4 rounded-lg border-r-4 border-[#F3752B]">
                              <p className="text-[#F3752B] font-semibold flex items-center">
                                <span className="ml-2 text-lg">↗️</span> {useCase.calloutText}
                              </p>
                            </div>
                          )}
                        </div>
